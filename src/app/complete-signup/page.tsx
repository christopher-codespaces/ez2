...
export default function CompleteSignupPage() {
  ...
  const uploadDriverDocuments = async (userId: string) => {
    if (!app) return [];

    const storage = getStorage(app);

    const uploads = await Promise.all(
      driverDocuments.map(async (file) => {
        const safeFileName = file.name.replace(/\s+/g, "-");
        const filePath = `driverDocuments/${userId}/${Date.now()}-${safeFileName}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!phone.trim() || !isValidZaPhone(phone)) {
      setError("Please enter a valid South African phone number, for example 0821234567 or +27821234567.");
      return;
    }
    ...
    try {
      ...
      if (role === "driver") {
        const uploadedDocuments = await uploadDriverDocuments(firebaseUser.uid);
        ...
        await updateDoc(ref, {
          ...
          businessLocation: "",
          businessDescription: "",
          driverDocuments: uploadedDocuments,
          updatedAt: Date.now(),
          ...
          {showDriverFields && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-black font-medium mb-1">
                    Upload documents, including police clearance *
                  </label>
                  ...
                  {!driverDocuments.length && (
                    <p className="text-sm text-gray-500 mt-1">
                      You can upload PDF, image, JPEG, PNG, DOC, or DOCX files.
                    </p>
                  )}
                  ...
                  {driverDocuments.length > 0 && (
                    <div className="mt-2">
                      ...
                      <ul className="mt-1 space-y-1">
                        {driverDocuments.map((file, idx) => (
                          <li key={`${file.name}-${idx}`} className="text-sm text-gray-700">
                            {file.name}
                          </li>
                        ))}
                      </ul>
